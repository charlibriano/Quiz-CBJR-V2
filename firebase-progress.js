...
const VERSION = 'cbjr-firebase-progress-core-v1';
...
function getTitleByXp(xp = 0) {
  ...
  if (value >= 5000) return '👑 Lenda CBJR';
...
function calculateStats(data = readLocal()) {
  const radioIndex = Number(data[STORAGE_KEYS.radioUnlocked] || 0);
  const radioAlbums = Math.max(0, Math.min(13, radioIndex));
  ...
  const xp = Number(data[STORAGE_KEYS.cbjrXp] || 0);
  const title = getTitleByXp(xp);
  return {
    xp,
    title,
    radioAlbums,
    letrasAlbums: Array.isArray(letras) ? letras.length : 0,
    radioAchievementsCount: Object.keys(radioAchievements || {}).filter(k => radioAchievements[k]).length,
    quizAchievementsCount: Object.keys(quizAchievements || {}).filter(k => quizAchievements[k]).length,
    medalsCount: Array.isArray(medals) ? medals.length : 0,
    medals: Array.isArray(medals) ? medals : [],
    progressPercent: Math.round(((radioAlbums / 13) + ((Array.isArray(letras) ? letras.length : 0) / 13)) / 2 * 100)
  };
}
